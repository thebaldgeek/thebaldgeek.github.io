...
This graph is critical to the ADSC ground station operator as it will show when the satellite is at the top of its orbit and at the bottom of its orbit. These are two very critical points of the orbit and you will be adjusting the dish at these points, so knowing ahead of time that you will need to setting an alarm for 3am and 3pm is very important. 
## Auto update TLE
...
Now that we know where the satellite is and when, we can start to set up the dish feed horn and pick up the 10500 signal.
The feedhorn we use is the Titanium Satellite C1-PLL Lite. It is important to use this exact feed horn for two reasons.
1. It will down convert the C-Band signal. This is important. Most LNBs are built for TV C-Band, the ADSC signal is at the bottom of the band, so most LNBs will not pick up the C-Band signal this low in frequncy.
...
It could be as small as an inch (25mm) up to 3-4 inches (75+mm). Thus you need to be as accurate as possible in your marks.   
...
This is critical and not optional. Mark the adjustable arm with a marker note the top of the orbit position and the bottom of the orbit position.  
...
Node-RED is outputting the ellevation position, we need to remap that to match the position required by the dish.   
...
Now, to sync all this up, put the arm on your desk on some paper marked with the length you measured off the back of the dish.  
...
Now cut the arm and mount the arm.   
